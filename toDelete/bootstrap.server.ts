...
import { getLogger as getLoggerInternal, resetLogger } from '@/core/logging/';
import { ErrorHandler, IErrorHandler } from '@/core/errors/';
...
import { createApiErrorInterceptor } from '@/core/errors/';
...
export async function bootstrapServer(): Promise<void> {
  try {
    ...
    } = await import('@/core/factories/serviceFactory.server');
    
    // Server-only initializations that use Node.js features
    const { getPrismaClient } = await import('@/core/factories/databaseFactory.server');
    const { 
      getUserRepository, 
    ...
    } = await import('@/core/factories/repositoryFactory.server');
...
export function resetServices(): void {
  ...
  import('@/core/factories/databaseFactory.server').then(({ resetPrismaClient }) => {
    resetPrismaClient();
  });
  
  import('@/core/factories/repositoryFactory.server').then(({ resetRepositories }) => {
    resetRepositories();
  });
  
  import('@/core/factories/serviceFactory.server').then(({ resetServices }) => {
    resetServices();
  });
}
 ...
 */
function getLogger(): import("@/core/logging/ILoggingService").ILoggingService {
