...
export class ClientTokenManager {
  ...
  static async setTokens(accessToken: string, refreshToken: string, expiresIn: number): Promise<void> {
    ...
    try {
      ...
      setItem('auth_expires_at', expirationTimestamp.toString());
      setItem('auth_expires_in', expiresIn.toString());
      setItem('auth_timestamp', now.toString());
      
      // Update token state
      TOKEN_STATE.tokenValue = accessToken;
      TOKEN_STATE.tokenExpiry = new Date(expirationTimestamp);
      TOKEN_STATE.lastOperation = now;
      
      // Notify about authentication change
      await this.notifyAuthChange(true);
      
      console.log('ClientTokenManager: Tokens set successfully');
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error('ClientTokenManager: Error setting tokens:', error);
  ...
  static async refreshAccessToken(): Promise<boolean> {
    ...
    // Minimum 10 seconds between refresh attempts to prevent refresh storms
    ...
    try {
      ...
      try {
        ...
        // Handle different response formats with enhanced data extraction
        let accessToken = null;
        let newRefreshToken = null;
        let expiresIn = 3600; // Default 1 hour
        
        // Format 1: {success: true, data: {token/accessToken, refreshToken}}
        if (data.success && data.data) {
          accessToken = data.data.token || data.data.accessToken;
          newRefreshToken = data.data.refreshToken;
          expiresIn = data.data.expiresIn || expiresIn;
        }
        // Format 2: {token/accessToken, refreshToken} (direct)
        else if (data.token || data.accessToken) {
          accessToken = data.token || data.accessToken;
          newRefreshToken = data.refreshToken;
          expiresIn = data.expiresIn || expiresIn;
        }
        // Format 3: Just check for success if no token found
        else if (data.success) {
          // The token might be set as a cookie only
          console.log('Token refresh succeeded but no token in response body, checking cookies');
            ...
            for (const cookie of cookies) {
              ...
              if (name === 'auth_token' && value) {
                try {
                  ...
                  console.log('Found auth_token in cookies after refresh');
                  break;
                } catch (e) {
                  console.warn('Error decoding auth_token cookie value:', e);
                }
              }
            }
          }
        ...
        if (!accessToken) {
          console.warn('Token refresh API returned no access token', {
            responseFormat: data ? Object.keys(data).join(',') : 'empty',
            hasSuccess: !!data?.success,
            hasData: !!data?.data,
            cookiesExist: typeof document !== 'undefined' ? document.cookie.length > 0 : false
          });
          return false;
        }
        
        // This validation is now done above with enhanced logging
