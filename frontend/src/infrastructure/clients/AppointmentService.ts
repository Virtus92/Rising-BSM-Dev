  ...
  */
  ...
  static async deleteAppointment(id: number): Promise<ApiResponse<void>> {
    try {
      ...
      console.error('Error in AppointmentService.deleteAppointment:', error);
      return {
        success: false,
        data: null,
  ...
  static async getCustomerAppointments(customerId: number, options?: AppointmentFilterParamsDto): Promise<ApiResponse<PaginatedResponse<AppointmentResponseDto>>> {
    try {
      ...
      console.error('Error in AppointmentService.getCustomerAppointments:', error);
      return {
        success: false,
        data: {
          data: [],
  ...
  static async getAppointmentStats(period?: string): Promise<ApiResponse<{
    ...
    try {
      ...
      console.error('Error in AppointmentService.getAppointmentStats:', error);
      return {
        success: false,
        data: null,
  ...
  static async addNote(id: number, note: string): Promise<ApiResponse<any>> {
    try {
      ...
      console.error('Error in AppointmentService.addNote:', error);
      return {
        success: false,
        data: null,
  ...
  static async getNotes(id: number, forceFresh = false): Promise<ApiResponse<any[]>> {
    try {
      ...
      console.error('Error in AppointmentService.getNotes:', error);
      return {
        success: false,
        data: [],
  ...
  static async count(filters?: {
    ...
    try {
      ...
      const query = queryParams.toString() ? `?${queryParams.toString()}` : '';
      return ApiClient.get(`${this.basePath}/count${query}`);
    } catch (error) {
      console.error('Error in AppointmentService.count:', error);
      return {
        success: false,
        data: 0,
