...
export const PATCH = routeHandler(
  async (req: NextRequest, context: { params: Promise<{ id: string }> }) => {
    ...
    const success = await apiKeyService.activateApiKey(apiKeyId, { userId: req.auth?.userId });

    if (!success) {
      return formatResponse.error('Failed to activate API key', 500);
    }

    return formatResponse.success({ activated: true });
  },
  { 
    requiresAuth: true,
