...
const PluginCard = ({ item, onActionClick }: CardProps<PluginDto>) => {
  ...
  const canDownload = hasPermission(SystemPermission.PLUGIN_DOWNLOAD);
    ...
    <BaseCard
      ...
      actions={
        <div className="flex justify-between gap-2">
          ...
          {canInstall && item.status === 'approved' && (
            <Button 
              variant="default" 
              size="sm" 
              onClick={() => onActionClick?.('install', item)}
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Discover and install plugins to extend your application
          </p>
        </div>
        {canCreate && (
          <Button
