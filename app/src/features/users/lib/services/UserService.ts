import { UserClient } from '@/features/users/lib/clients/UserClient';
import { 
  UserDto, 
  CreateUserDto,
  UpdateUserDto,
...
export class UserService {
  ...
  static async getUsers(filters?: UserFilterParamsDto): Promise<ApiResponse<PaginationResult<UserDto>>> {
    try {
      ...
      while (retries <= maxRetries) {
        try {
          ...
          return {
            ...
            message: response.message,
            data: response.data as PaginationResult<UserDto>,
            statusCode: response.statusCode
          };
        } catch (requestError) {
          retries++;
      ...
      console.error('Error in UserService.getUsers:', error as Error);
      return {
        success: false,
        data: {
          data: [],
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if ('data' in responseData && typeof responseData.data === 'object') {
          ...
          const userData = responseData.data as UserDto;
          return {
            ...response,
            data: userData
          };
      ...
      console.error(`Error fetching user ${id}:`, error as Error);
      return {
        success: false,
