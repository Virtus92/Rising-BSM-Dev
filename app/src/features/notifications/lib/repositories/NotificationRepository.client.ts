/**
 * Simple notification client repository
 * Designed to work with the client without server dependencies
 */
...
export class NotificationRepository implements INotificationRepository {
  ...
  async deleteAllForUser(userId: number): Promise<number> {
    // Not implemented in client repository
    return 0;
  }
  ...
  async transaction<T>(work: (repository: INotificationRepository) => Promise<T>): Promise<T> {
    // Transactions are not supported in client repository
    return work(this);
  }
}
