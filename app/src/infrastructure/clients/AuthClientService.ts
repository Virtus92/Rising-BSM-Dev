...
/**
 * Generic API response type with success flag, optional message, and typed data
 */
...
class AuthClientServiceImpl implements IAuthService {
  ...
  async refreshToken(
    ...
    try {
      // Handle both string and RefreshTokenDto formats
      const payload = typeof refreshTokenDto === 'string' 
      ...
      // Return just the data part to match the interface
      return response.data;
    } catch (error) {
  ...
  async logout(
    ...
    try {
      const response = await ApiClient.post('/auth/logout', logoutDto);
