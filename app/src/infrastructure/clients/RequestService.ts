...
export class RequestService {
  ...
  static async count(filters?: {
    ...
    try {
      const response = await this.countRequests(filters);
      ...
      if (response.success) {
        ...
        if (response.data && typeof response.data === 'object' && 'count' in response.data) {
          return {
            ...response,
            data: { count: (response.data as unknown as { count: number }).count }
          };
        }
        
        // If response.data is an object with a total property, normalize it
        if (response.data && typeof response.data === 'object') {
          // Use type assertion to access 'total' property safely
          const dataObject = response.data as unknown as { total?: number };
          if (dataObject.total !== undefined) {
            return {
              ...response,
              data: { count: dataObject.total }
            };
          }
        }
        
        // If we couldn't determine the count format, return 0
      ...
      // For error responses, ensure we return the correct format
      return {
        success: false,
        data: { count: 0 },
        message: response.message || 'Error retrieving count',
