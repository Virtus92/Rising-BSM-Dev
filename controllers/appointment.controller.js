...
class AppointmentController extends BaseController {
  ...
  async getAllAppointments(req, res, next) {
    return this.executeQuery(async () => {
      ...
      if (date) {
        conditions.push(`DATE(t.termin_datum) = $${paramCounter++}`);
        params.push(date);
      }
  ...
  async getAppointmentById(req, res, next) {
    return this.executeQuery(async () => {
      ...
      const appointmentQuery = await ConnectionManager.withConnection(client => client.query({
        text: `
        SELECT 
          t.*, 
          k.name AS kunde_name,
   ...
   */
  async addAppointmentNote(req, res, next) {
    return this.executeQuery(async () => {
      const { id } = req.params;
      const { note } = req.body;
